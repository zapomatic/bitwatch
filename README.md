...
- Telegram notifications for balance changes
  ...
  - Choose where address links in alerts point (your configured mempool instance, public mempool.space, or plain text) and whether Telegram shows link previews, on the Integrations page.
- Configure auto-acceptance or alert mode of balance changes (chain_in, chain_out, mempool_in, mempool_out). New installs alert on all four by default: self-hosted mempool instances backed by Fulcrum/electrum report spends as a reduced `chain_in` rather than a `chain_out`, so ignoring inputs would miss outgoing funds.
