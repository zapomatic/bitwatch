...
const attemptCall = async (addr, testResponse) => {
  return new Promise((resolve, reject) => {
    ...
    const req = (api.protocol === "https:" ? https : http).request(
      ...
      function (res) {
        ...
        res.on("end", function () {
          ...
          logger.success(
            `Balance ${addr}: chain (in=${result.actual.chain_in}, out=${result.actual.chain_out}), mempool (in=${result.actual.mempool_in}, out=${result.actual.mempool_out})`
          );
          resolve(result);
        });
      }
    ...
    req.on("error", (e) => {
      if (e.statusCode === 504) {
        logger.warning(
          `Server capacity exceeded for ${addr} - address will be ignored`
        );
...
export default async (addr, onRateLimit, testResponse) => {
  let retryCount = 0;
  const maxRetries = 5;
  ...
  while (retryCount < maxRetries) {
    ...
    if (
      ...
      if (retryCount === maxRetries) {
        logger.error(
          `Rate limit exceeded for ${addr} after ${maxRetries} retries`
        );
        return {
          error: true,
      ...
      logger.warning(
        `Rate limited for ${addr}, retrying in ${delay}ms (attempt ${retryCount}/${maxRetries})`
      );
